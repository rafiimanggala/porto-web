...
/* Same skeleton as ScrollScene (pinned track, sticky stage, chapter header, spring-smoothed
   ...
   The outgoing text is fully gone at the swap point and the incoming text starts from there, so
   the two never share a frame. The swap point sits one FADE before the chapter marker, so the
   incoming text is whole exactly at the marker and a tick jump never lands on a faded caption. */
...
/* Opacity and y for one caption: in over the FADE after start - EARLY, out over the FADE before end - EARLY. */
function useSwap({ p, start, end }: Swap) {
  const outB = end === null ? 0 : end - EARLY;
  const outA = outB - FADE;
  const inA = start === null ? 0 : start - EARLY;
  const xs: number[] = [];
  const op: number[] = [];
  const dy: number[] = [];
  if (start !== null) {
...
type Sw = (i: number) => Swap;

function ChapterHeader({ p, captions, chapters, readout, swap, onGo }: {
  p: MV;
  captions: readonly SceneCaption[];
  chapters: readonly number[];
  readout?: (p: MV) => ReactNode;
...
function VisualBox({ p, render }: { p: MV; render: (p: MV) => ReactNode }) {
  return (
...
function CaptionStack({ captions, swap }: { captions: readonly SceneCaption[]; swap: Sw }) {
  return (
...
export default function GeneticsSceneShell({
  captions,
  chapters,
  render,
  readout,
  ...
  const swap: Sw = (i) => ({ p, start: i === 0 ? null : chapters[i], end: i === last ? null : chapters[i + 1] });
    ...
    <>
      <div ref={track} className={`relative motion-reduce:hidden ${heightClass}`}>
        <div className="sticky top-14 h-[calc(100svh-3.5rem)] overflow-hidden">
          <ChapterHeader p={p} captions={captions} chapters={chapters} readout={readout} swap={swap} onGo={goTo} />
          <div className="mx-auto grid h-full max-w-[1180px] grid-rows-[minmax(0,1fr)_auto] gap-3 px-4 pb-4 pt-[52px] sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] sm:grid-rows-1 sm:gap-10 sm:px-8 sm:pb-8 sm:pt-16">
            <VisualBox p={p} render={render} />
            <CaptionStack captions={captions} swap={swap} />
          </div>
        </div>
      </div>
